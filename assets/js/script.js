document.addEventListener("DOMContentLoaded", function() {
    ...
    const proxyUrl = "https://www.whateverorigin.org/get?url=";

    const bootstrapUrl = proxyUrl + encodeURIComponent("https://fantasy.premierleague.com/api/bootstrap-static/") + "&callback=?";
    const fixturesUrl = proxyUrl + encodeURIComponent("https://fantasy.premierleague.com/api/fixtures/") + "&callback=?";

    // Fetch current gameweek data
    $.getJSON(bootstrapUrl, function(data) {
        ...
        if (currentGameweekId) {
            ...
            $.getJSON(fixturesUrl, function(fixturesData) {
                fixturesData = JSON.parse(fixturesData.contents);
                const currentGameweekFixtures = fixturesData.filter(fixture => fixture.event === currentGameweekId);
